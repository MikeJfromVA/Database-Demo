...
-- Count rows, group by customer
SELECT COUNT(*)
FROM INVOICE
GROUP BY CUS_CODE
;

-- Shows customers and how many invoices
SELECT CUS_CODE, COUNT(*)
FROM INVOICE
GROUP BY CUS_CODE
;

-- cannot print out the thing you are counting
-- SO
-- any column in SELECT must be in GROUP BY
/*
SELECT CUS_CODE, inv_number, COUNT(*)
FROM INVOICE
GROUP BY CUS_CODE
;
*/

-- Can combine with joins
SELECT CUSTOMER.CUS_FNAME, CUSTOMER.CUS_LNAME, COUNT(*) || ' invoices'
FROM INVOICE
JOIN CUSTOMER
...
-- Don't do this - Don't pretty-print things

-- Speaking of pretty-print, here is a view
CREATE VIEW CUSTOMER_TOTALS AS
SELECT CUSTOMER.CUS_FNAME, CUSTOMER.CUS_LNAME, COUNT(*) || ' invoices' AS TotalInvoices
FROM INVOICE
JOIN CUSTOMER
ON CUSTOMER.CUS_CODE = INVOICE.CUS_CODE
GROUP BY CUSTOMER.CUS_CODE
;
-- In a view, it is sometimes OK to pretty-print
-- That query also uses an alias

-- Treat a view just like a table
SELECT *
FROM CUSTOMER_TOTALS
;

-- arbitrary math expressions work
SELECT INV_NUMBER, SUM(LINE_PRICE * LINE_UNITS)
FROM LINE
GROUP BY INV_NUMBER
ORDER BY INV_NUMBER
;
-- This gets the total order total of each invoice


-- total spent per product
SELECT PRODUCT.P_DESCRIPT, SUM(LINE.LINE_PRICE * LINE.LINE_UNITS)
FROM LINE
JOIN PRODUCT
ON PRODUCT.P_CODE = LINE.P_CODE
GROUP BY PRODUCT.P_DESCRIPT
