...
-- Counts all rows where cus_initial is NON NULL
SELECT COUNT(CUS_INITIAL)
FROM CUSTOMER
;
...
-- EXPLICIT Join (MORE POWERFUL)
SELECT CUSTOMER.CUS_FNAME, INVOICE.INV_NUMBER
FROM CUSTOMER
JOIN INVOICE
...
-- LEFT JOIN to view unmatched customers
SELECT CUSTOMER.CUS_FNAME, INVOICE.INV_NUMBER
FROM CUSTOMER 
LEFT JOIN INVOICE
ON CUSTOMER.CUS_CODE = INVOICE.CUS_CODE
...
-- RIGHT JOIN to view unmatched products
SELECT DISTINCT PRODUCT.P_CODE, LINE.LINE_PRICE
FROM LINE 
RIGHT JOIN PRODUCT
...
-- We can use ON and WHERE
-- ON is the first filter
...
ORDER BY PRODUCT.P_CODE
;
-- Returns all products have never been ordered
