...
-- Aggregate function. Sums the column for the ENTIRE table
select sum(product.p_price) from product;

-- Aggregate function, adds all prices for each vendor code
select product.v_code, max(product.p_price) 
from product
group by product.v_code
order by v_code
;

-- Some products do not have vendors
select * from product where product.v_code is null;

-- Join and then show ONLY the unmatched rows from the left table
select * 
...
;
-- It does not work! The SELECT is supposed to produce a list (one column)
...
-- Can sometimes replace an inner query with a join
select distinct customer.cus_fname
from customer, invoice
where customer.cus_code = invoice.cus_code
;

-- Example using LIMIT, descending sorting, sum with operations
select customer.cus_code, sum(line.line_units*line.line_price) as spent
