...
;
-- is also EQUIJOIN because COL=COL (equality check)
-- is also NATURAL JOIN because COL=COL both called "cus_code"
...
-- EXPLICIT INNER JOIN THREE TABLES
select customer.cus_fname, customer.cus_lname, line.p_code
from customer
join invoice
on customer.cus_code = invoice.cus_code
join line
...
;
-- NOTHIN FROM INVOICE. JUST NEEDED IT TO RELATE CUSTOMER TO LINE

-- EXPLICIT INNER JOIN FOUR TABLES
select customer.cus_fname, customer.cus_lname, product.p_descript
from customer
join invoice
on customer.cus_code = invoice.cus_code
join line
...
;
-- NOTHIN FROM INVOICE. JUST NEEDED IT TO RELATE CUSTOMER TO LINE
-- NOTHIN FROM LINE. JUST NEEDED IT TO GET TO PRODUCT

-- Just for fun, check the list of names by themselves
...
select customer.cus_lname, customer.cus_fname from customer 
order by customer.cus_lname, customer.cus_fname;

-- So, the FOUR TABLE joins above only shows purchased products ALONG WITH customers who purchased
-- Which MIGHT be what you want

-- What if you want to show ALL customers ALONG WITH their purchased products
-- SLIGHT difference:
select customer.cus_fname, customer.cus_lname, product.p_descript
from customer
LEFT join invoice
...
-- SIMPLER (TWO TABLES) LEFT JOIN:
select customer.cus_fname, customer.cus_lname, invoice.inv_number
from customer LEFT join invoice
on customer.cus_code = invoice.cus_code
;

-- Left & Right refer to table order as typed
select customer.cus_fname, customer.cus_lname, invoice.inv_number
from invoice RIGHT join customer
on customer.cus_code = invoice.cus_code
;

select p_descript from product;

select v_name from vendor;

-- FULL Rescues orphan (unmatched) rows from BOTH columns
