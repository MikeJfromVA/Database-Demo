...
-- UNION JOIN (vertical instead of horizontal)
...
-- SELF JOIN
SELECT 
VENDOR_LEFT.V_NAME AS Vendor_1, 
VENDOR_RIGHT.V_NAME AS Vendor_2, 
VENDOR_LEFT.V_AREACODE AS AreaCode
FROM VENDOR AS VENDOR_LEFT -- Requires an alias
FULL OUTER JOIN VENDOR AS VENDOR_RIGHT -- Requires an alias
ON VENDOR_LEFT.V_AREACODE = VENDOR_RIGHT.V_AREACODE
AND VENDOR_LEFT.V_NAME != VENDOR_RIGHT.V_NAME
ORDER BY VENDOR_LEFT.V_AREACODE, VENDOR_LEFT.V_NAME, VENDOR_RIGHT.V_NAME
;
...
ORDER BY PRODUCT.P_DESCRIPT
;

-- Which products have NEVER been ordered?
-- ... using IS NULL
SELECT DISTINCT PRODUCT.P_DESCRIPT
FROM LINE RIGHT JOIN PRODUCT
ON LINE.P_CODE = PRODUCT.P_CODE
WHERE LINE.P_CODE IS NULL
ORDER BY PRODUCT.P_DESCRIPT
;
...
-- IN is short for A=1 OR A=2 OR A=3...
SELECT VENDOR.V_NAME
FROM VENDOR
WHERE VENDOR.V_AREACODE IN ('901', '904')
;

-- List of all customers living in same area code as a vendor
-- Uses a SUBQUERY in WHERE CLAUSE
SELECT DISTINCT CUSTOMER.CUS_FNAME, CUSTOMER.CUS_LNAME
FROM CUSTOMER
WHERE CUSTOMER.CUS_AREACODE IN (SELECT VENDOR.V_AREACODE FROM VENDOR)
;

-- Originally, the inner query was a list of customers, together with all products purchased + vendor
-- We filter it to show only customers who bought products that had NO vendor
-- Using a subquery in the FROM clause
SELECT *
FROM -- subquery functions as our table
(SELECT DISTINCT CUSTOMER.CUS_FNAME, CUSTOMER.CUS_LNAME, PRODUCT.P_DESCRIPT, VENDOR.V_NAME
FROM CUSTOMER
...
ORDER BY CUS_LNAME, CUS_FNAME, P_DESCRIPT) AS CUSTOMER_PRODUCTS
WHERE V_NAME IS NULL -- check for null
AND P_DESCRIPT IS NOT NULL -- check for not null
AND CUS_FNAME IS NOT NULL -- check for not null
;
