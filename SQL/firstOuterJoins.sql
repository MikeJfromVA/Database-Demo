OP TABLE IF EXISTS LINE CASCADE;
...
-- Explicit five-table join
-- Shows all products bought by all customers (includes product vendor)
...
LEFT JOIN VENDOR -- Some products have no Vendor!
ON VENDOR.V_CODE = PRODUCT.V_CODE
ORDER BY CUS_LNAME, CUS_FNAME, P_DESCRIPT
;

-- Left Outer Join
-- Customers and Invoices INCLUDING Customers with NO INVOICES
SELECT DISTINCT CUSTOMER.CUS_FNAME, CUSTOMER.CUS_LNAME, INVOICE.INV_NUMBER
...
ON LINE.INV_NUMBER = INVOICE.INV_NUMBER
FULL OUTER JOIN PRODUCT -- Gets inactive customers AND unwanted products
ON PRODUCT.P_CODE = LINE.P_CODE
LEFT OUTER JOIN VENDOR -- Gets products w no vendor AND inactive customers
ON VENDOR.V_CODE = PRODUCT.V_CODE
ORDER BY CUS_LNAME, CUS_FNAME, P_DESCRIPT
;
